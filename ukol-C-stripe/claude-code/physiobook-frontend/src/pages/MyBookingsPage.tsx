...
export default function MyBookingsPage() {
  ...
  useEffect(() => {
    ...
    if (payment === 'success') {
      showToast('Platba byla uspesne provedena!', 'success');
      setSearchParams({}, { replace: true });
      refetch();
    } else if (payment === 'cancelled') {
      showToast('Platba byla zrusena.', 'error');
      setSearchParams({}, { replace: true });
    }
  ...
  const handlePay = async (bookingId: string) => {
    ...
    try {
      const { sessionUrl } = await bookingService.createCheckoutSession(bookingId);
      window.location.href = sessionUrl;
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Nepodarilo se vytvorit platebni session';
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          {(() => {
                            const ps = paymentStatusConfig[bookingItem.paymentStatus] || paymentStatusConfig[PaymentStatus.Unpaid];
                            return (
                              <span
                                className={`inline-flex items-center px-2.5 py-0.5 text-xs font-medium rounded-full border ${ps.className}`}
                              >
                                {ps.label}
                              </span>
                            );
                      ...
                      <div className="flex gap-2 flex-shrink-0">
                        {canPay(bookingItem.status, bookingItem.paymentStatus) && (
                          <button
                            ...
                            disabled={payingBookingId === bookingItem.id}
                          ...
                          >
                            {payingBookingId === bookingItem.id ? 'Presmerovani...' : 'Zaplatit'}
                          </button>
                        )}
