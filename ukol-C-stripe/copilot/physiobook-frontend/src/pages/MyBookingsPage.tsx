...
import { useBookings, useCancelBooking, useCreateCheckoutSession } from '@/features/bookings/hooks';
...
export default function MyBookingsPage() {
  ...
  const { createCheckoutSession, loading: redirectingToCheckout } = useCreateCheckoutSession();
  ...
  const canCancel = (status: BookingStatus) =>
    status === BookingStatus.Pending || status === BookingStatus.Confirmed || status === BookingStatus.Paid;

  const canPay = (status: BookingStatus, paymentStatus: PaymentStatus) =>
  ...
  const handlePay = async (bookingId: string) => {
    ...
    showToast('Nepodarilo se otevrit platebni branu.', 'error');
  };
          ...
          {data.items.length === 0 ? (
            ...
            <div className="space-y-4">
              {data.items.map((bookingItem) => {
                const status = statusConfig[bookingItem.status] || statusConfig[BookingStatus.Pending];
                const paymentStatus =
                  paymentStatusConfig[bookingItem.paymentStatus] || paymentStatusConfig[PaymentStatus.Pending];
                return (
                  <div
                    key={bookingItem.id}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 text-xs font-medium rounded-full border ${paymentStatus.className}`}
                          >
                            {paymentStatus.label}
                          </span>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {canPay(bookingItem.status, bookingItem.paymentStatus) && (
                          <button
                            onClick={() => handlePay(bookingItem.id)}
                            disabled={redirectingToCheckout}
                            className="px-4 py-2 text-sm font-medium text-white bg-teal-600 border border-teal-600 rounded-lg hover:bg-teal-700 disabled:opacity-50 transition-colors flex-shrink-0"
                          >
                            Zaplatit
                          </button>
                        )}
