...
import { useAvailability, useCreateBooking, useCreateCheckoutSession } from '@/features/bookings/hooks';
...
export default function BookingPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (result) {
      const checkout = await createCheckoutSession(result.id);
      if (checkout?.checkoutUrl) {
        window.location.href = checkout.checkoutUrl;
        return;
      }

      showToast('Rezervace byla vytvorena, ale nepodarilo se otevrit platbu.', 'error');
            ...
            >
              {booking || checkoutLoading ? (
                <span className="inline-flex items-center justify-center gap-2">
                  ...
                  {booking ? 'Vytvarim rezervaci...' : 'Presmerovani na platbu...'}
                </span>
              ) : (
